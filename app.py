...
# Kartläggning av språk till Cardmarket språk-ID
LANG_MAP = {
    "ENG": 1,
    "FRA": 2,
    "GER": 3,
    ...
    "POR": 8,
    "KOR": 9,
    "ZHT": 10 # Traditional Chinese
}

# Kartläggning av skick till Cardmarket condition-ID
COND_MAP = {
    "MT": 1, # Mint
    "NM": 2, # Near Mint
    "EX": 3, # Excellent
...
def generate_cardmarket_url(card_name, set_name, set_nr, lang_code, cond_code):
    """ Skapar en filtrerad Cardmarket-länk """
...
# --- API SÖKNING (POKÉMON TCG API) ---
@st.cache_data(ttl=3600)
def search_pokemon_cards(query):
    if not query:
        return []
    try:
        url = f"https://api.pokemontcg.io/v2/cards?q=name:\"{query}*\" OR number:\"{query}\""
...
with tab1:
    ...
    if collection:
        ...
        # Hantera Radera
        with st.expander("🛠️ Hantera / Radera kort", expanded=False):
...
with tab2:
    ...
    search_query = st.text_input("Skriv kortnamn eller setnummer (t.ex. 'Charizard', 'Raichu' eller '016'):", key="card_search_input")
    ...
    if search_query:
        ...
        if results:
            ...
            # Skapa ett kort-nätverk för snabbt val
            for card_api in results[:12]: # Visa max 12 åt gången för prestanda
                with st.container():
                    ...
                    # Hämta estimerat Cardmarket EUR-värde om tillgängligt från API:et
                    ...
                    with col_form:
                        with st.form(key=f"add_form_{card_api['id']}"):
                            ...
                            if st.form_submit_button("➕ Lägg till i samlingen", type="primary", use_container_width=True):
                                ...
                                # Knuffa fram pärmnummer om det redan finns
            ...
            st.warning("Hittade inga kort som matchade sökningen. Prova att söka på enbart namnet.")
