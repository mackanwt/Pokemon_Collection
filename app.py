...
def process_uploaded_image(file):
    ...
    try:
        img = Image.open(file)
        img = ImageOps.exif_transpose(img) # Roterar rätt om mobilbilden togs snett
        img = img.convert("RGB")
        img.thumbnail((400, 400)) # Komprimerar bilden så att den är lätt att spara
...
@st.dialog("🎴 Kortdetaljer & Hantera bild")
def show_card_dialog(selected_index, card_data):
    ...
    uploaded_file = st.file_uploader("📷 Ta kort / Välj bild från mobilen", type=["jpg", "jpeg", "png"], key=f"uploader_{selected_index}")
    
    if uploaded_file is not None:
        st.image(uploaded_file, caption="Förhandsvisning av ny bild", width=180)
        if st.button("💾 Spara ny bild på kortet", type="primary", use_container_width=True):
            b64_img = process_uploaded_image(uploaded_file)
            if b64_img:
                app_data["collection"][selected_index]["Bild"] = b64_img
                save_data_to_github(app_data)
                st.success("Bilden har sparats!")
                st.rerun()

# --- HUVUDLAYOUT ---
tab1, tab2, tab3 = st.tabs(["📊 Samling", "✏️ Redigera text", "➕ Lägg till nytt kort"])

# --- FLIK 1: HUVUDSAMLING (Med Bildvisning) ---
with tab1:
    ...
    if not collection_df.empty:
        ...
        column_config = {
            ...
            "Köpt för (SEK)": st.column_config.NumberColumn("Köpt (SEK)", width="small", format="%.2f"),
            ...
            "Värde idag (SEK)": st.column_config.NumberColumn("Värde idag (SEK)", width="medium", format="%.2f"),
        }

        # Tabellvisning för läsläge
        st.dataframe(
            df_display,
        ...
        # Knapp för att ta/ändra bild för ett befintligt kort
        col_sel, col_btn = st.columns([1, 1])
        with col_sel:
            selected_row_num = st.number_input("🖼️ Välj radnummer för att lägga till/ändra bild:", min_value=1, max_value=len(df_display), value=1, step=1)
        with col_btn:
            st.write("")
            st.write("")
            if st.button("📷 Ta/Ändra bild på valt kort", use_container_width=True):
                idx = selected_row_num - 1
                show_card_dialog(idx, app_data["collection"][idx])
...
# --- FLIK 2: REDIGERINGSLÄGE FÖR TEXT (Inga bilder i tabellen) ---
with tab2:
    st.subheader("✏️ Massredigera text och priser")
    collection_df = pd.DataFrame(app_data.get("collection", []))
    
    if not collection_df.empty:
        # Skapa en kopia utan "Bild"-kolumnen så att inte data_editor raderar den
        text_df = collection_df.drop(columns=["Bild"], errors="ignore")
        
        columns_text_order = [
            "Pärmnummer", "Språk", "Namn", "Setnr.", "SetBet.", "Set", 
            "Övrigt", "Skick", "Köpt för (EUR)", "Värde (EUR)", "Datum tillagd"
        ]

        edited_df = st.data_editor(
            text_df,
            column_order=columns_text_order,
            num_rows="dynamic",
            use_container_width=True,
            key="text_editor_only"
        )
        
        if st.button("💾 Spara alla textändringar till GitHub", type="primary", use_container_width=True):
            new_records = edited_df.to_dict(orient="records")
            
            # Bevara de sparade bilderna när texten uppdateras
            for i, record in enumerate(new_records):
                if i < len(app_data["collection"]):
                    record["Bild"] = app_data["collection"][i].get("Bild", "")
                else:
                    record["Bild"] = ""
            
            app_data["collection"] = new_records
            save_data_to_github(app_data)
...
with tab3:
    ...
    uploaded_new_file = st.file_uploader("📷 Välj bild / Ta foto för nytt kort", type=["jpg", "jpeg", "png"], key="new_card_file")
    ...
    with st.form("add_new_card_form"):
        ...
        if st.form_submit_button("⚡ Spara nytt kort i samlingen", type="primary", use_container_width=True):
            img_b64 = process_uploaded_image(uploaded_new_file) if uploaded_new_file else ""
